...
class Parser:
	def __init__(self, tokens):
		self.tokens = tokens
		self.tok_idx = -1
		self.need_to_continue_parsing = False
		self.advance()
	...
	def statements(self):
		...
		while True:
			...
			if not more_statements and not self.need_to_continue_parsing: break
			self.need_to_continue_parsing = False
	...
	def expr(self):
		...
		if self.current_tok.matches(TT_KEYWORD, 'let'):
			...
			expr = res.register(self.expr())
			if res.error: return res
			self.need_to_continue_parsing = True
	...
	def power(self):
		return self.bin_op(self.squares, (TT_POW, ), self.factor)
	...
	def squares(self):
		return self.bin_op(self.call, (TT_LSQUARE, ), self.squares_op)
	...
	def if_expr_cases(self, case_keyword):
		...
		if self.current_tok.type == TT_LBRACE:
			...
			if res.error: return res
			new_cases, else_case = all_cases
			if not new_cases and not else_case:
				self.reverse()
